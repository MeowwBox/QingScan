...
<div class="flex justify-between items-start mb-6 p-6 pb-0">
    <div>
        <h1 class="text-2xl font-bold text-slate-800 mb-2">GitHub监控通知</h1>
        <nav class="flex gap-2 text-sm text-slate-500">
            ...
            <span class="text-slate-300">/</span>
            <span class="text-slate-700 font-medium">监控通知</span>
        </nav>
    </div>
    <div class="flex gap-3">
        <a href="<?php echo url('webscan/github_keyword_monitor/index')?>" class="px-5 py-2.5 rounded-xl border border-slate-300 text-slate-600 font-medium hover:bg-slate-50 transition-all duration-200">
...
<div class="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm mx-6">
    ...
    <div class="flex justify-between items-center px-5 py-4 border-b border-slate-100 bg-slate-50/50">
        <div class="flex items-center gap-3">
            <h2 class="text-lg font-bold text-slate-800">监控结果</h2>
        ...
        <table class="w-full">
            <thead>
                <tr class="bg-slate-50 border-b border-slate-100">
                    ...
                    <th class="px-5 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">名称</th>
                    <th class="px-5 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">文件路径</th>
                    <th class="px-5 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">URL地址</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-slate-100">
                <?php foreach ($list as $value) { ?>
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td class="px-5 py-4 text-slate-500 text-sm font-mono"><?php echo $value['path'] ?></td>
                        <td class="px-5 py-4">
                            <a href="<?php echo $value['html_url'] ?>" target="_blank" class="text-blue-500 hover:text-blue-600 hover:underline text-sm">
