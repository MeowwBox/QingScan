...
<main class="p-6 min-h-screen">
    ...
    <div class="flex justify-between items-start mb-6">
        <div>
            <h1 class="text-2xl font-bold text-slate-800 mb-2">未授权访问详情</h1>
            <nav class="flex gap-2 text-sm text-slate-500">
                <a href="/" class="hover:text-blue-500 transition-colors">首页</a>
                <span class="text-slate-300">/</span>
                <a href="<?php echo url('unauthorized/index') ?>" class="hover:text-blue-500 transition-colors">未授权访问</a>
                <span class="text-slate-300">/</span>
        ...
        <div class="flex gap-2">
            ...
            <?php }?>
            <a href="<?php echo url('unauthorized/index') ?>" class="px-4 py-2.5 rounded-xl border border-slate-300 text-slate-600 font-medium hover:bg-slate-50 transition-colors">返回列表</a>
            <a href="<?php echo url('unauthorized/details', ['id' => $info['upper_id']]) ?>" class="px-4 py-2.5 rounded-xl border border-slate-300 text-slate-600 font-medium hover:bg-slate-50 transition-colors">上一页</a>
            <a href="<?php echo url('unauthorized/details', ['id' => $info['lower_id']]) ?>" class="px-4 py-2.5 rounded-xl border border-slate-300 text-slate-600 font-medium hover:bg-slate-50 transition-colors">下一页</a>
        </div>
    </div>

    <input type="hidden" id="to_examine_url" value="<?php echo url('to_examine/unauthorized')?>">
    ...
    <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        ...
        <div class="flex justify-between items-center px-6 py-5 border-b border-slate-100 bg-slate-50">
            <div class="flex items-center gap-3">
                <h2 class="text-xl font-bold text-slate-800">未授权访问详情</h2>
                <span class="bg-amber-100 text-amber-600 text-xs px-2.5 py-1 rounded-full font-medium">安全风险</span>
            </div>
        ...
        <div class="p-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                ...
                <div class="bg-slate-50 rounded-xl p-4 border border-slate-200">
                    <span class="text-xs text-slate-500 uppercase tracking-wider">IP</span>
                    <div class="mt-1 font-mono text-slate-700"><?php echo $info['ip'] ?? '-' ?></div>
                </div>
                <div class="bg-slate-50 rounded-xl p-4 border border-slate-200">
                    <span class="text-xs text-slate-500 uppercase tracking-wider">Port</span>
                    <div class="mt-1">
            ...
            <div class="bg-slate-50 rounded-xl border border-slate-200 overflow-hidden">
                <table class="w-full">
                    <tbody class="divide-y divide-slate-200">
                        ...
                        <tr>
                            <td class="px-5 py-4 text-sm font-semibold text-slate-500 bg-slate-100">描述</td>
                            <td class="px-5 py-4 text-slate-700"><?php echo $info['text'] ?? '-' ?></td>
                        </tr>
                        <tr>
