...
<div class="max-w-2xl mx-auto py-8">
    <div class="bg-white border border-slate-200 rounded-2xl shadow-card p-8">
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-slate-800">添加API任务</h1>
            <p class="text-slate-500 text-sm mt-1">配置API扫描任务参数</p>
        </div>
        <form method="post" action="/index.php?s=urls/_add_api_url">
            <div class="space-y-5">
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">JSON数据内容</label>
                    <textarea name="json_data" class="w-full bg-slate-50 border border-slate-300 rounded-xl px-4 py-3 text-slate-800 placeholder-slate-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 focus:outline-none transition-all resize-none font-mono text-sm" rows="8" placeholder="请输入JSON数据"></textarea>
                </div>
            </div>
